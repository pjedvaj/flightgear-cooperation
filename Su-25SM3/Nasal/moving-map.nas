...
var setupCanvas = func {
	...
	#window.getCanvas(1).addPlacement({"node": "ti_screen", "texture": "mm.png"});
...
var maps_base = getprop("/sim/fg-home") ~ '/cache/mapsSu25SM3';
...
#	    }
	...
	new: func {
      	...
      	foreach(var name; keys(mm.input)) {
        	mm.input[name] = props.globals.getNode(mm.input[name], 1);
      	}
      	
      	mm.setupCanvasSymbols();
      	
      	#map
      	mm.lat = mm.input.latitude.getValue();
		mm.lon = mm.input.longitude.getValue();
      	mm.mapSelfCentered = TRUE;
      	mm.day = TRUE;
      	mm.setupMap();
		...
		# steerpoints
		mm.newSteerPos = nil;
		mm.showSteers = TRUE;#only for debug turn to false
		mm.showSteerPoly = TRUE;#only for debug turn to false
	...
	loop: func {
		...
		if (me.off == TRUE) {
			setprop("controls/mfd/displays/brightness-mm", 0);
			settimer(func{me.loop();},0.5);
			return;
		} else {
			setprop("controls/mfd/displays/brightness-mm", me.brightness);
					...
					(func {# generator function
					    ...
					    if( io.stat(img_path) == nil and me.liveMap == TRUE) { # image not found, save in $FG_HOME
					          ...
					          .fail(func (r) {#print('Failed to get image ' ~ img_path ~ ' ' ~ r.status ~ ': ' ~ r.reason);
					          				tile.set("src", "Aircraft/Su-25/Models/Instruments/MFD/MovingMap/emptyTile.png");
					      					tile.update();
					      					});
					    	...
					    	tile.set("src", "Aircraft/Su-25/Models/Instruments/MFD/MovingMap/noiseTile.png");
					      	tile.update();
					    }
					})();
		  		}
