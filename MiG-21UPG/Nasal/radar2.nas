...
# Every 1 second (Edited - Alex Barrett 171011):
...
var RadarStandbyMP    = props.globals.getNode("sim/multiplay/generic/int[6]");
...
# from debug.nas for use in 1.9.1
var isnan = func {
...
var rdr_loop = func() {
	...
	if (cnt_hud == 0.1) {
                var nada = RadarStandby.getValue();
	        if ( RadarStandbyMP != nil ) {
			RadarStandbyMP.setIntValue(nada); # Tell over MP if our radar is scaning or is in stanby.
			};
		hud_nearest_tgt();
		cnt_hud = 0;
	} else {
