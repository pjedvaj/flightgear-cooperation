...
# liveries =========================================================
#aircraft.livery.init("Aircraft/bk117-b2/Models/liveries");
...
var turbine = props.globals.getNode("sim/model/bk117/turbine-rpm-pct", 1);
var torque_pct = props.globals.getNode("sim/model/bk117/torque-pct", 1);
...
var startup_proc = props.globals.initNode("/engines/startup_proc","BOOL");
startup_proc.setValue(0);
var startup_proc = props.globals.initNode("/engines/shutdown_proc","BOOL");
startup_proc.setValue(0);
...
var procedure = {
	...
	next: func(delay = 0) {
		...
		# interpolate(startup_proc,startup_proc.getValue()+delay,delay);
	},
	process: func(id) {
		id == me.loopid or return;
		# startup
		if (me.stage == 1) {
			print("", "1: press start button #1 -> spool up turbine #1 to N1 8.6--15%");
			...
			print("", "2: move power lever #1 forward -> fuel injection");
			engines.engine[0].powerN.setValue(0.13);
			me.next(2.5);

		} elsif (me.stage == 3) {
			print("", "3: turbine #1 ignition (wait for EGT stabilization)");
			me.next(4.5);

		} elsif (me.stage == 4) {
			print("", "4: move power lever #1 to idle position -> engine #1 spools up to N1 63%");
			engines.engine[0].powerN.setValue(0.63);
			me.next(5);

		} elsif (me.stage == 5) {
			print("", "5: release start button #1\n");
			engines.engine[0].starterN.setValue(0);
			engines.engine[0].ignitionN.setValue(0);
			me.next(3);

		} elsif (me.stage == 6) {
			print("", "6: press start button #2 -> spool up turbine #2 to N1 8.6--15%");
			engines.engine[1].ignitionN.setValue(1);
			engines.engine[1].starterN.setValue(1);
			me.next(5);

		} elsif (me.stage == 7) {
			print("", "7: move power lever #2 forward -> fuel injection");
			engines.engine[1].powerN.setValue(0.13);
			me.next(2);

		} elsif (me.stage == 8) {
			print("", "8: turbine #2 ignition (wait for EGT stabilization)");
			me.next(5);

		} elsif (me.stage == 9) {
			print("", "9: move power lever #2 to idle position -> engine #2 spools up to N1 63%");
			engines.engine[1].powerN.setValue(0.63);
			me.next(8);

		} elsif (me.stage == 10) {
			print("", "10: release start button #2\n");
			setprop("engines/startup_proc",0);
			engines.engine[1].starterN.setValue(0);
			engines.engine[1].ignitionN.setValue(0);
			me.next(1);

		} elsif (me.stage == 11) {
			print("", "11: move both power levers forward -> turbines spool up to 100%");
			engines.engine[0].powerN.setValue(1);
			engines.engine[1].powerN.setValue(1);

		# shutdown
		} elsif (me.stage == -1) {
			print("", "-1: power lever in idle position; cool engines");
...
var vibration = { # and noise ...
	...
	update: func(dt) {
		...
		if (airspeed > 115) { # overspeed vibration
			var frequency = 2000 + 500 * rand();
			var v = 0.49 + 0.5 * normatan(airspeed - 120, 10);
			var intensity = v;
...
#aircraft.livery.init("Aircraft/bk117/Models/Variants", "sim/model/bk117/name");
