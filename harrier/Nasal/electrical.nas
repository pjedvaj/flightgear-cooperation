...
var last_test 		= nil;
var switches 		= nil;
var input_component 	= nil;
var output 		= nil;
var input_volts 	= nil;		
var scheme 		= nil;				
var output_component 	= nil;
var output_props 	= nil;   
var name 		= nil;    
var input 		= nil;
var supplier 		= nil;
...
var update = func {
  ...
  foreach(var supplier; suppliers) {
    volts = 0;
    ideal_volts = components[supplier].getNode("volts").getValue();		# Not very realistic, but good enough
     kind = components[supplier].getNode("kind").getValue();
    if (kind == "alternator") {
      min = components[supplier].getNode("source-min").getValue();
     source_val = getprop(components[supplier].getNode("source-prop").getValue());
  ...
  foreach(var connector; connectors) {
    closed = 1;								# Circuit begins closed
    last_test = 0;
    switches = connector.getChildren("switch");				# Begin testing all associated switches
   ...
   input_component = components[connector.getNode("input").getValue()];
    output = connector.getNode("output").getValue();
    input_volts = 0;							# Default to an open circuit
    if (closed) {								# Switches all tested positive
      ...
      scheme = connector.getNode("scheme").getValue();			# Scheme stuff:
    ...
    if (input_volts > components_lastvolts[output]) {				# Update only if input > previous inputs
      ...
      output_props = output_component.getChildren("prop");
      for (var i=0; i<size(output_props); i+=1) {				# Set component's outputs to new volts
if(getprop("/systems/electrical/serviceable") == 1){				# Set component's outputs to new volts
        setprop(output_props[i].getValue(),input_volts);

	}else{
		setprop(output_props[i].getValue(),0.0);
	}
      }
...
var init = func {
  ...
  for(var i=0; i<size(component_list); i+=1) {
    ...
    name = component_list[i].getNode("name").getValue();
    ...
    kind = component_list[i].getNode("kind").getValue();
    if (kind != "battery" and
        kind != "alternator" and
        kind != "external" and
        kind != "output") {
  ...
  for(var i=0; i<size(connector_list); i+=1) {
    ...
    input = connector_list[i].getNode("input").getValue();
    ...
    output = connector_list[i].getNode("output").getValue();
