var AcModel        	= props.globals.getNode("sim/model");
...
var ROOT = props.globals.getNode("sim/weapons");
var name = nil;
var T 			= nil;
var p 			= nil;
var rho 		= nil;
...
### Todo:
#see for physics: https://gitorious.org/fg/flightgear/source/a59e04ff3669d3e160f96464567f758b068ec562:src/AIModel/AIBallistic.cxx#L649

var MISSILE = {
        new : func (p, sel, xxx, yyy, zzz) {
		...
		m.zpos		= zzz;
			   ...
			   setprop("models/model[" ~ i ~ "]/path", m.missile_model);
        ...
        del: func {
                ...
                if(me.free == 1 and me.Tgt != nil){
                        ...
                        var phrase = me.NameOfMissile ~ " missed " ~ me.Tgt.get_Callsign();
                        if (getprop("sim/mp-messaging")=="true") {
        ...
        release: func() {
                ...
                #interpolate(me.rollN.getPath(), 45, 1);
                me.StartTime = props.globals.getNode("/sim/time/elapsed-sec", 1).getValue();

                var phrase =  me.NameOfMissile ~ " fired at " ~ me.Tgt.get_Callsign();
                if (getprop("sim/mp-messaging")=="true") {
                        setprop("/sim/multiplay/chat", phrase);
                } else {
                        setprop("/sim/messages/atc", phrase);
        ...
        update: func {
                ...
                if(me.rail=="true"){			
			setprop("controls/armament/missile-sound", 1);
                        if (me.life_time > 0) { 
				f_lbs = me.force_lbs * 2 ;
			}
                        if (me.life_time > 4) { 
                ...
                # rho is adjusted for altitude in rho_sndspeed(altitude),
                ...
                if ( me.status == 2 ) {
                        ...
                        if(ground != nil){
                                if(ground>alt_ft){
                                        print(me.NameOfMissile ~ " hit the ground");
                                        me.free = 1;
...
#between the position A and the position B, the distance x to the target is enough short to proxiimity detection.
                ...
                if ( me.direct_dist_m != nil ) {
                 ...
                 if ( cur_dir_dist_m > me.direct_dist_m and me.direct_dist_m< me.maxExplosionRange * 2) {
					...
					setprop("sim/multiplay/generic/int[9]", 1);
                                        print("MISSILE: Distance of impact: ",  me.direct_dist_m, " Time: ",getprop("sim/time/elapsed-sec"));
                                        impact_report(me.t_coord, wh_mass, "missile"); # pos, alt, mass_slug,(speed_mps)
                                        var phrase =   me.NameOfMissile ~ " exploded " ~ sprintf( "%2.1f", me.direct_dist_m) ~ "m from " ~ me.Tgt.get_Callsign();
                                        if (getprop("sim/mp-messaging")=="true") {
                                                setprop("/sim/multiplay/chat", phrase);
                                        } else {
                                                setprop("/sim/messages/atc", phrase);
        ...
        search: func (c){
                               ...
                               var abs_dev_deg = math.abs(total_horiz);
                                ...
                                me.TgtLon_prop       = me.Tgt.get_Longitude;
                                me.TgtLat_prop       = me.Tgt.get_Latitude;
                                me.TgtAlt_prop       = me.Tgt.get_altitude;
                                me.TgtHdg_prop       = me.Tgt.get_heading;
...
##To be corrected...
	...
	animate_explosion: func {
		if(getprop("controls/armament/missile-explode") == 1){
			setprop("controls/armament/missile-explode2", 1);
			setprop("controls/armament/missile-explode", 0);
		}elsif(getprop("controls/armament/missile-explode2") == 1){
			setprop("controls/armament/missile-explode2", 0);
			setprop("controls/armament/missile-explode", 1);
		}
	    me.reload_model(me.missile_Explosion);
		settimer(func{
			me.reload_model("Aircraft/harrier/Models/Effects/Explosion/ImpactFireSmoke.xml");
		}, 0.3);
    },
...
var MPReport = func () {
        if (getprop("sim/mp-messaging")=="true") {
                setprop("sim/mp-messaging","false");
        }else{
                setprop("sim/mp-messaging","true");
        }
        var phrase = "MP messaging : " ~ getprop("sim/mp-messaging");
	gui.popupTip(phrase, 7);
}
...
var rho_sndspeed = func(altitude) {
	...
	snd_speed = math.sqrt( 1.4 * 1716 * (T + 459.7));
	return [rho, snd_speed];

}

var load_missile_settings = func(search_name){
	foreach(var weapon; ROOT.getChildren("weapon")){
		name = weapon.getNode("name").getValue();
		if(name == search_name){
			setprop("controls/armament/missile/address",		weapon.getNode("smoke_path").getValue());
  			setprop("controls/armament/missile/addressNoSmoke", 	weapon.getNode("model_path").getValue());
  			setprop("controls/armament/missile/addressExplosion",	weapon.getNode("explosion_path").getValue());
  			setprop("controls/armament/missile/max-detectionrngnm",	weapon.getNode("max_detection_rng_nm").getValue());
  			setprop("controls/armament/missile/fov-deg",		weapon.getNode("fov_deg").getValue());
  			setprop("controls/armament/missile/detection-fov-deg",	weapon.getNode("detection_fov_deg").getValue());
  			setprop("controls/armament/missile/track-max-deg",	weapon.getNode("track_max_deg").getValue());
  			setprop("controls/armament/missile/thrust-lbs",		weapon.getNode("thrust_lbs").getValue());
 			setprop("controls/armament/missile/max-g",		weapon.getNode("max_g").getValue());
  			setprop("controls/armament/missile/weight-launch-lbs",	weapon.getNode("weight_launch_lbs").getValue());
  			setprop("controls/armament/missile/thrust-duration-sec",weapon.getNode("thrust_duration_sec").getValue());
  			setprop("controls/armament/missile/weight-warhead-lbs",	weapon.getNode("weight_warhead_lbs").getValue());
  			setprop("controls/armament/missile/drag-coeff",		weapon.getNode("drag_coeff").getValue());
  			setprop("controls/armament/missile/drag-area",		weapon.getNode("drag_area").getValue());
  			setprop("controls/armament/missile/maxExplosionRange",	weapon.getNode("max_explosion_range").getValue());
  			setprop("controls/armament/missile/maxspeed",		weapon.getNode("max_speed").getValue());
  			setprop("controls/armament/missile/life",		weapon.getNode("life").getValue());
  			setprop("controls/armament/missile/fox",		weapon.getNode("fox").getValue());
  			setprop("controls/armament/missile/rail",		weapon.getNode("rail").getValue());
  			setprop("controls/armament/missile/cruise_alt",		weapon.getNode("cruise_alt").getValue());
		}
	}
}